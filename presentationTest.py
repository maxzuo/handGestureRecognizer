...
def findLocalMax(values):
	...
	for i in range(0, len(changedValues) - 3):
...
def findRefindMax(values):
	...
	for i in range(0, len(changedValues) - 4):
		if changedValues[i] - changedValues[i+1] < -3:
			directions.append(-1)
		elif changedValues[i] - changedValues[i+1] > 3:
			directions.append(1)
...
while True:
	...
	if type(roiThreshold) != type(None):
		_, contours, _= cv2.findContours(roiThreshold, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)
	...
	if fingerValue == prevValue and type(fingerValue) != type(None):
		...
		elif (datetime.now() - prevTime).seconds >= 3 and not talked:
			thread.start_new_thread(talk, tuple([fingerValue]))
			talked = True
			print "called talk"
		elif (datetime.now() - prevTime).seconds >= 3 and talked:
			prevTime = datetime.now()
	else:
		talked = False
		output = ""
	...
	# cv2.imshow("Highlighted Band", thresholdedRes)
